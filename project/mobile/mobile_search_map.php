<?php
	...
	$result = mysqli_query($con, "SELECT be.ship_id, be.beacon_floor, be.center_width, be.center_height FROM beacon be WHERE be.beacon_major IN
		(SELECT ba.request_major from bangle ba where ba.bangle_id IN (SELECT us.bangle_id FROM user us WHERE us.user_name = '$user_name' and us.user_parent = '$bangle_id' and ba.ship_id = be.ship_id))");
	$response = array();
